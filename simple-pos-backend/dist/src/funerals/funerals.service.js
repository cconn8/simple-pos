...
let FuneralsService = class FuneralsService {
    ...
    async create(data) {
        const funeralRecord = data.funeralData
            ? {
    ...
    async findByIdAndUpdate(id, updateFuneralDto) {
        let updateData;
        if (updateFuneralDto.funeralData) {
            updateData = {
                $set: {
                    funeralData: updateFuneralDto.funeralData,
                    ...(updateFuneralDto.paymentStatus && { paymentStatus: updateFuneralDto.paymentStatus })
                },
                $unset: { formData: 1 }
            };
        }
        ...
        const updatedDoc = this.funeralModel.findByIdAndUpdate(id, updateData, { new: true });
        return updatedDoc;
    }
